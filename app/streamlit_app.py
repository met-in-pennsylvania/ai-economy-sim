...
if st.session_state.last_run is not None:
    ...
    st.success(f"Ran **{run['scenario_name']}** — {len(history)} quarters in {run['elapsed']:.1f}s")
    ...
    ur = final.total_unemployed / max(1, lf)
    lfpr = lf / max(1, total_all)
    labor_share = final.total_labor_income / max(1.0, final.nominal_gdp)
    init_gdp = history[0].nominal_gdp

    col1.metric("Real GDP growth", f"{(final.real_gdp / max(1, history[0].real_gdp) - 1) * 100:.1f}%")
    col2.metric("Unemployment rate", f"{ur * 100:.1f}%")
    col3.metric("LFP rate", f"{lfpr * 100:.1f}%")
    col4.metric("Labor share of GDP", f"{labor_share * 100:.1f}%")
